...
public class FrameAdminEditarFuncionario extends JFrame implements ActionListener, WindowListener, ItemListener {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					FrameAdminEditarFuncionario frame = new FrameAdminEditarFuncionario(
							(Funcionario) EpontoFachada.getInstance().getPessoaNome("Ikaro"));
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource().equals(btnLimpar)) {
			...
			if (txtSenha.getText().equals("")) {
				...
				try {
					empresa = (Empresa) EpontoFachada.getInstance()
							.buscaEmpresaNome(cmbBxEmpresa.getSelectedItem().toString());
				} catch (EmpresaNaoEncontradaException e3) {
					JOptionPane.showMessageDialog(null, e3.getMessage());
				}
				LocalTime horaChegada = LocalTime.of(Integer.parseInt(txtHoraChegada.getText()),
						Integer.parseInt(txtMinutosChegada.getText()));
				LocalTime horaSaida = LocalTime.of(Integer.parseInt(txtHoraSaida.getText()),
						Integer.parseInt(txtMinutosSaida.getText()));
				LocalTime horaChegadaIntervalo = LocalTime.of(Integer.parseInt(txtHoraChegadaIntervalo.getText()),
						Integer.parseInt(txtMinutosChegadaIntervalo.getText()));
				LocalTime horaSaidaIntervalo = LocalTime.of(Integer.parseInt(txtHoraSaidaIntervalo.getText()),
						Integer.parseInt(txtMinutosSaidaIntervalo.getText()));
				try {
					if (chkbxAlterarFoto.isSelected()) {
						funcionarioNew = new Funcionario(nome, cpf, email, senha, telefone, empresa, cargo,
								"Seg. à Sex", horaChegada, horaSaida, horaChegadaIntervalo, horaSaidaIntervalo);
				...
				try {
					EpontoFachada.getInstance().editarPessoa(
							EpontoFachada.getInstance().getIndiceCpf(funcionario.getCpf()), funcionarioNew);
