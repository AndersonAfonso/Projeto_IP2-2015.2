...
public class RegPonto implements Serializable {
	...
	public boolean isChegadaCorreta() { // Chegada correta com tolerância de 15
										// minutos
		...
		for (int i = 0; i <= 15; i++) {
			if (ponto.equals(this.funcionario.getChegada())) {
				...
				else if (ponto.isAfter(this.funcionario.getChegada()))
	...
	public boolean isSaidaCorreta() {// Saida correta com tolerância de 15
										// minutos
		LocalTime ponto = LocalTime.of(agora.getHour(), agora.getMinute());
		boolean retorno = false;
		...
		for (int i = 0; i <= 15; i++) {
			if (ponto.equals(funcionario.getSaida())) {
				...
				if (ponto.isBefore(this.funcionario.getSaida()))
					ponto = ponto.plusMinutes(1);
				else if (ponto.isAfter(this.funcionario.getSaida()))
					ponto = ponto.minusMinutes(1);
			}
		}
		return retorno;
	...
	public boolean isIntervalo_InCorreta() { // Ida pro Intervalo correta com
		...
		for (int i = 0; i <= 15; i++) {
			if (ponto.equals(funcionario.getIntervalo_in())) {
				...
				if (ponto.isBefore(this.funcionario.getIntervalo_in()))
					ponto = ponto.plusMinutes(1);
				else if (ponto.isAfter(this.funcionario.getIntervalo_in()))
					ponto = ponto.minusMinutes(1);
			}
		}
		return retorno;
	...
	public boolean isIntervalo_OutCorreta() { // volta do Intervalo correta com
												// tolerância de 15 minutos
		LocalTime ponto = LocalTime.of(agora.getHour(), agora.getMinute());
		boolean retorno = false;
		...
		for (int i = 0; i <= 15; i++) {
			if (ponto.equals(funcionario.getIntervalo_out())) {
				...
				if (ponto.isBefore(this.funcionario.getIntervalo_out()))
					ponto = ponto.plusMinutes(1);
				else if (ponto.isAfter(this.funcionario.getIntervalo_out()))
					ponto = ponto.minusMinutes(1);
			}
		}
		return retorno;
